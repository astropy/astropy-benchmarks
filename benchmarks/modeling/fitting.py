...
Gaussian2D = models.Gaussian2D(amplitude=1, x_mean=0, y_mean=0, x_stddev=1, y_stddev=1)
...
combined_gauss_2d = models.Gaussian2D(
    amplitude=1, x_mean=0, y_mean=0, x_stddev=0.1, y_stddev=0.1
...
for i in range(20):
    ...
    large_gauss_combined_2d += models.Gaussian2D(
        x_mean=mean, y_mean=mean, x_stddev=stddev, y_stddev=stddev
    )
...
z_base = 3 * np.exp(
    -0.5 * ((x_grid - 1.3) ** 2 / 0.8**2 + (y_grid - 2.1) ** 2 / 0.1**2)
)
...
hbeta_combo = (
    models.Gaussian1D(34, 4862.721, 5)
...
fluxes = np.arange(1.0 * width * height).reshape(width, height)
...
def time_Gaussian1D_LevMarLSQFitter():
    ...
    fit_LevMarLSQFitter(Gaussian1D, x, y)
...
def time_Gaussian1D_SLSQPLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_SLSQPLSQFitter(Gaussian1D, x, y)
    except Warning:
        pass
...
def time_Gaussian1D_SimplexLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_SimplexLSQFitter(Gaussian1D, x, y)
    except Warning:
        pass
...
def time_Gaussian2D_SLSQPLSQFitter():
    ...
    try:
        z = z_base + np.random.normal(0.0, 0.2, z_base.shape)
        fit_SLSQPLSQFitter(Gaussian2D, x_grid, y_grid, z)
    except Warning:
        pass
...
def time_Polynomial1D_LevMarLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_LevMarLSQFitter(Polynomial1D, x, y)
    except Warning:
        pass
...
def time_Polynomial1D_LinearLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_LinearLSQFitter(Polynomial1D, x, y)
    except Warning:
        pass
...
def time_Polynomial2D_LevMarLSQFitter():
    ...
    try:
        z = z_base + np.random.normal(0.0, 0.2, z_base.shape)
        fit_LevMarLSQFitter(Polynomial2D, x_grid, y_grid, z)
    except Warning:
        pass
...
def time_Chebyshev1D_LinearLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_LinearLSQFitter(Chebyshev1D, x, y)
    except Warning:
        pass
...
def time_Chebyshev2D_LevMarLSQFitter():
    ...
    try:
        z = z_base + np.random.normal(0.0, 0.2, z_base.shape)
        fit_LevMarLSQFitter(Chebyshev2D, x_grid, y_grid, z)
    except Warning:
        pass
...
def time_combined_gauss_1d_LevMarLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_LevMarLSQFitter(combined_gauss_1d, x, y)
    except Warning:
        pass
...
def time_combined_gauss_1d_SLSQPLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_SLSQPLSQFitter(combined_gauss_1d, x, y)
    except Warning:
        pass
...
def time_combined_gauss_2d_LevMarLSQFitter():
    ...
    try:
        z = z_base + np.random.normal(0.0, 0.2, z_base.shape)
        fit_LevMarLSQFitter(combined_gauss_2d, x_grid, y_grid, z)
    except Warning:
        pass
...
def time_combined_gauss_2d_SLSQPLSQFitter():
    ...
    try:
        z = z_base + np.random.normal(0.0, 0.2, z_base.shape)
        fit_SLSQPLSQFitter(combined_gauss_2d, x_grid, y_grid, z)
    except Warning:
        pass
...
def time_datafit_compound_LevMarLSQFitter():
    warnings.filterwarnings("error")
    try:
        fit_LevMarLSQFitter(hbeta_combo, wave, flux)
    except Warning:
        pass
...
def time_large_gauss_combined_1d_SLSQPLSQFitter():
    ...
    try:
        y = y_base + np.random.normal(0.0, 0.2, y_base.shape)
        fit_SLSQPLSQFitter(large_gauss_combined_1d, x, y)
    except Warning:
        pass
...
def time_multi_model_LinearLSQFitter():
    ...
    try:
        y = pixels.T
        fit_LinearLSQFitter(line, x=t, y=y)
    except Warning:
        pass
